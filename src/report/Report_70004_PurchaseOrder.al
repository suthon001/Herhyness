/// <summary>
/// Report PurchaseOrder (ID 70004).
/// </summary>
report 70004 "MRC Purchase Order"
{
    Caption = 'Purchase Order';
    DefaultLayout = RDLC;
    RDLCLayout = './LayoutReport/LCLReport/Report_70004_PurchaseOrder.rdl';
    UsageCategory = None;
    dataset
    {
        dataitem(PurchaseHeader; "Purchase Header")
        {
            ...
            column(CreateDocBy; SalePerson.Name) { }
            ...
            column(Fax; Vendor."Fax No.") { }
            column(UserSetUPCard_Signature; TenantMedia.Content) { }
            ...
            begin
                ...
                if not PaymentTerm.GET("Payment Terms Code") then
                    PaymentTerm.init();
                if not SalePerson.GET("Purchaser Code") then
                    SalePerson.Init();
                if Status = Status::Released then begin
                    approveEntry.reset();
                    approveEntry.SetCurrentKey("Entry No.");
                    approveEntry.SetRange("Document Type", "Document Type");
                    approveEntry.SetRange("Document No.", "No.");
                    approveEntry.SetRange(Status, approveEntry.Status::Approved);
                    if approveEntry.FindLast() then begin
                        if not UserSetUPCard.GET(approveEntry."Approver ID") then
                            UserSetUPCard.Init();
                        if UserSetUPCard."YVS Signature".Count <> 0 then
                            if TenantMedia.GET(UserSetUPCard."YVS Signature".Item(1)) then
    ...
    var
        ...
        UserSetUPCard: Record "User Setup";
